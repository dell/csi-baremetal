...
func DefineVolumeExpansionTests(driver *baremetalDriver) {
	type local struct {
	...
	ginkgo.BeforeEach(func() {
		// Check preconditions.
		if !driver.GetDriverInfo().Capabilities[testsuites.CapBlock] && pattern.VolMode == v1.PersistentVolumeBlock {
			e2eskipper.Skipf("Driver %q does not support block volume mode - skipping", driver.GetDriverInfo().Name)
		}
		if !driver.GetDriverInfo().Capabilities[testsuites.CapControllerExpansion] {
	...
	init := func() {
		...
		testVolumeSizeRange := e2evolume.SizeRange{Min: "1Gi"}
