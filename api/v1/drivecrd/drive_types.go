...
func (in *Drive) GetDriveDescription() string {
	spec := in.Spec
	description := fmt.Sprintf("Drive Details: SN='%s', Model='%s %s', Slot='%s', Type='%s', Size='%d', NodeId='%s'",
		spec.SerialNumber, spec.VID, spec.Slot, spec.PID, spec.Type, spec.Size, spec.NodeId)
