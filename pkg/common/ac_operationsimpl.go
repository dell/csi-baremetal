...
// UpdateACSizeOrDelete updates size of AC or delete that AC if new size is low then some threshold
// or if AC storage class in not related to LVG
// bytes - difference in size, could be negative number (decrease size)
func (a *ACOperationsImpl) UpdateACSizeOrDelete(ac *accrd.AvailableCapacity, bytes int64) error {
	ctx, fn := context.WithTimeout(context.Background(), 10*time.Second)
	defer fn()

	if ac.Spec.StorageClass == api.StorageClass_HDDLVG || ac.Spec.StorageClass == api.StorageClass_SSDLVG {
		// try to modify size only for AC based on LVG
		newSize := ac.Spec.Size + bytes
		if newSize > acSizeMinThresholdBytes {
			a.log.WithField("method", "updateACSizeOrDelete").
				Infof("Updating size of AC %s to %d bytes", ac.Name, newSize)
			ac.Spec.Size = newSize
			return a.k8sClient.UpdateCR(ctx, ac)
		}
	}
...
func (a *ACOperationsImpl) recreateACToLVGSC(sc api.StorageClass, acs ...*accrd.AvailableCapacity) *accrd.AvailableCapacity {
	...
	if err = a.k8sClient.CreateCR(context.Background(), lvg, name); err != nil {
	...
	if err = a.k8sClient.CreateCR(context.Background(), newACCR, newACCRName); err != nil {
