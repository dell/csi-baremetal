...
func (m *VolumeManager) checkWbtChangingEnable(ctx context.Context, vol *volumecrd.Volume) bool {
	if m.wbtConfig == nil {
		m.log.Infof("cyx debug: volume: %s checkWbtChangingEnable wbtconfig is nil", vol.Name)
		return false
	}

	if !m.wbtConfig.Enable {
		m.log.Infof("cyx debug: volume: %s checkWbtChangingEnable wbtconfig.Enable is NOT set", vol.Name)
		return false
	}
