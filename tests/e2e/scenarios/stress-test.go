...
// 		ssPods, err := f.PodClientNS(ns).List(ctx, metav1.ListOptions{LabelSelector: fmt.Sprintf("sts=%s", stsName)})
...
// 		common.CleanupAfterCustomTest(ctx, f, driverCleanup, nil, pvcPointersList)
// 	}

// 	ginkgo.It("should serve StatefulSet on multi node cluster", func(ctx context.Context) {
// 		init()
// 		ginkgo.DeferCleanup(cleanup(ctx))
