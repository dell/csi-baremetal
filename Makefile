...
run-csi-baremetal-functional-tests:
	...
	sed -i '/parser.addoption("--hosts", action="store", default=\[\], help="Hosts")/s/default=\[\],/default=\['"$$edited_list"'\],/' ${PROJECT_DIR}/tests/e2e/conftest.py; \
	...
	sed -i '/parser.addoption("--ansible_server", action="store", default="", help="Server")/s/default="",/default="${ANSIBLE_SERVER_IP}",/' ${PROJECT_DIR}/tests/e2e/conftest.py; \
	if [ ${REGRESSION_JOB_ENABLE} == "true" ]; then \
		sed -i '/parser.addoption("--cmo_bundle_version", action="store", default="", help="Version of CMO bundle")/s/default="",/default="'"$$LATEST_BUNDLE_VERSION"'",/' ${PROJECT_DIR}/tests/e2e/conftest.py; \
	fi; \
	echo "conftest.py:"; \
